                                             ...
                                             const std::string &p_name)
{
    ...
    try
    {
        ...
        {
            std::string request = "CREATE TABLE IF NOT EXISTS _migrations(id SERIAL, file_name VARCHAR(255), "
                                  "table_name VARCHAR(255), applied_at TIMESTAMP DEFAULT NOW())";
...
void DatabaseManager::show_status() {

    std::string request = "SELECT file_name, table_name FROM _migrations";
    ...
    try {
        ...
        auto result = transaction.exec(request);

        for (auto row : result) {
            auto file_name = row[0].as<std::string>();
            auto table_name = row[1].as<std::string>();

            std::string cur = std::format("{} -> {}\n", file_name, table_name);
            output.append(cur);
        }
        ...
        Logger::getInstance().error("Cannot show status");
    }
}
...
std::vector<std::string> DatabaseManager::get_modified_tables(const std::string &request)
{
    ...
    std::regex r(
        R"((?:TABLE|INTO|DELETE\s+FROM|UPDATE|FROM|INSERT\s+INTO|JOIN|TRUNCATE\s+TABLE|ALTER\s+TABLE)\s+(?:IF\s+NOT\s+EXISTS\s+)?(\w+))");
...
void DatabaseManager::execute(std::string &mig_name, std::string &mig_type)
{
    ...
    try
    {
        ...
        auto request_file = std::ifstream(std::format("{}/migrations/{}.{}.sql", current_path.string(), mig_name, mig_type));
        ...
        if (request_file)
        {
...

            {
                ...
                std::string check_request =
                    std::format("SELECT file_name FROM _migrations WHERE file_name='{}'", mig_name);
                ...
                if (!check_transaction_result.empty())
                {
                    ...
                    else
                    {
                        ...
                        std::string migr_requets =
                            std::format("DELETE FROM _migrations WHERE file_name='{}'", mig_name);
                ...
                else
                {
                    if (mig_type == "up")
                    {
                        ...
                        {
                            std::string migr_requst = std::format(
                                "INSERT INTO _migrations(file_name, table_name) VALUES ('{}', '{}')", mig_name, table);
                ...
                for (auto &table : modified_tables)
                {
                    std::string check_table =
                        std::format("SELECT table_name FROM information_schema.tables WHERE table_name='{}'", table);

                    auto check_table_res = transaction.exec(check_table);

                    if (check_table_res.empty())
                    {
                        std::string delete_table_request =
                            std::format("DELETE FROM _migrations WHERE table_name='{}'", table);
